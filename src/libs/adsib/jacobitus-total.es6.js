class JacobitusTotal {
    constructor() {
        this.urlBase = 'https://localhost:9000/api';
    }

    async verificarEjecucion() {
                ...
                if (data.finalizado) {
                    return ({
                        ...
                        datos: {
                            ...
                            apiVersion: data.datos.api_vension
                        }
                    });
                } else {
                    return ({
            ...
            .then(data => {
                if (data.exito) {
                    return ({
                        ...
                        mensaje: data.mensaje,
                        datos: {
                            certificados: data.datos.dispositivo.certificados.filter((item) => item.tipo === 'X509_CERTIFICATE')
                        }
                    })
                } else {
                    return ({
    ...
    async firmarPdfModoSeguro(ci, pdfBase64) {
        ...
        return await fetch(`${this.urlBase}/token/sign`, {
            ...
            body: JSON.stringify({ ci, pdfs: [{ id: 'documento.pdf', pdf: pdfBase64 }] })
        })
            .then(response => response.json())
            .then(data => {
                if (data.datos.pdfs_firmados && data.datos.pdfs_firmados.length > 0) {
                    return ({
                        exito: true,
                        mensaje: data.mensaje,
                        datos: {
    ...
    async firmarJsonModoSeguro(ci, jsonBase64) {
        return await fetch(`${this.urlBase}/token/sign`, {
            ...
            body: JSON.stringify({ ci, jsons: [{ id: 'documento.json', json: jsonBase64 }] })
        })
            .then(response => response.json())
            .then(data => {
                if (data.datos.jsons_firmados && data.datos.jsons_firmados.length > 0) {
                    return ({
                        exito: true,
                        mensaje: data.mensaje,
                        datos: {
    ...
    async validarPdf(pdfBase64Firmado) {
        pdfBase64Firmado = pdfBase64Firmado?.replace('data:application/pdf;base64,', '');
